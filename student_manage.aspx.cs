    ...
    public partial class student_manage : System.Web.UI.Page
    {
        ...
        private void reloadList(GridView list, DataTable ds)
        {
            if (ds.Rows.Count == 0)
            {
                ...
                list.Rows[0].Cells[0].Text = " No data available.";
            }
            else
            {
                list.DataSource = ds;
        ...
        protected void addBt_Click(object sender, EventArgs e)
        {
            ...
            if (res == 1)
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "addSuccess", "Toast.fire({icon: 'success', title: 'Added successfully'})", true);
                reloadSearchStudentList();
            }
            else
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "addFail", "Toast.fire({icon: 'error', title: 'Failed to add'})", true);
            }
        }
        ...
        protected void search_studentList_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            if (res == 1)
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "addSuccess", "Toast.fire({icon: 'success', title: ' Deleted successfully'})", true);
                reloadSearchStudentList();
            }
            else
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "addFail", "Toast.fire({icon: 'error', title: ' Failed to delete'})", true);
            }
        }
        ...
        protected void search_studentList_RowEditing(object sender, GridViewEditEventArgs e)
        {
            ...
            birth = birth.Replace("Year", "-");
            birth = birth.Replace("Month", "-");
            birth = birth.Replace("Day", "");
            DataTable classs = dbc.excList("SELECT classid,name FROM class ORDER BY classid");
            reloadSearchStudentList();
        ...
        protected void search_studentList_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            if (res == 1)
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "addSuccess", "Toast.fire({icon: 'success', title: 'Updated successfully'})", true);
            }
            else
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "addFail", "Toast.fire({icon: 'error', title: ' Failed to update'})", true);
            }
        }
    }
}