...
namespace score_m
{
    public partial class score_manage : System.Web.UI.Page
    {
        ...
        protected void addBt_Click(object sender, EventArgs e)
        {
            ...
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "tip", "Toast.fire({icon: 'info', title: ' Please enter a number'})", true);
                return;
            }
            ...
            if (res == 1)
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "addSuccess",
                    "Toast.fire({icon: 'success', title: 'Added successfully'})", true);
                reloadStudentScore();
            }
            else
            {
