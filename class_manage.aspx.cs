        ...
        protected void classList_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            ...
            if (res == 1)
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "updateSuccess",
                    "Toast.fire({icon: 'success', title: 'Updated successfully'})", true);
                reloadClassList();
            }
            else
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "updateFail", "Toast.fire({icon: 'error', title: 'Failed to update'})", true);
            }
        }
        ...
        protected void classList_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            if (res == 1)
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "deleteSuccess",
                    "Toast.fire({icon: 'success', title: 'Deleted successfully'})", true);
                reloadClassList();
            }
            else
            {
                ScriptManager.RegisterStartupScript(this, GetType(), "deleteFail", "Toast.fire({icon: 'error', title: 'Failed to delete'})", true);
            }
        }
